# app/modules/openphish_check.py

import requests
from app.modules.base import DetectionModule

class OpenPhishModule(DetectionModule):
    FEED_URL = "https://openphish.com/feed.txt"

    def __init__(self, email_data):
        super().__init__(email_data)
        self._phish_urls = []

    def _fetch_feed(self):
        try:
            response = requests.get(self.FEED_URL, timeout=10)
            if response.status_code == 200:
                self._phish_urls = response.text.strip().splitlines()
        except Exception as e:
            # Suppress crash: feed failure just disables module
            self._phish_urls = []

    def run(self):
        self._fetch_feed()

        if not self._phish_urls:
            return None

        flags = []
        score = 0

        for link in self.parsed_email.get("links", []):
            for phish_url in self._phish_urls:
                if phish_url.strip() and phish_url.strip() in link:
                    score += 50
                    flags.append(f"⚠️ Link matches OpenPhish: {phish_url.strip()}")
                    break  # Stop after first match per link

        return {
            "score": min(score, 100),
            "flags": flags
        }
